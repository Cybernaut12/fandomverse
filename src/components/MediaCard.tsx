...
import { Star, Bookmark, Play, Plus, Check, Film, Tv, Gamepad2, Sparkles, BookOpen, Layers } from 'lucide-react';
...
export const MediaCard: React.FC<MediaCardProps> = ({ item, showProgress = true }) => {
  const { 
    setSelectedMedia, 
    toggleBookmark, 
    isBookmarked,
  ...
  const [imgSrc, setImgSrc] = useState<string>(item.coverImage);
  ...
  return (
    <div 
      ...
      className="group relative rounded-xl glass-card overflow-hidden cursor-pointer flex flex-col justify-between transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-black/60 border border-white/[0.08] hover:border-white/20"
    >
      ...
      <div className="relative aspect-[2/3] w-full overflow-hidden bg-slate-900">
        {!hasError ? (
          <img
            ...
            className="w-full h-full object-cover object-top transition-transform duration-500 group-hover:scale-105 filter brightness-[0.95]"
          />
        ) : (
          <div className="w-full h-full bg-gradient-to-br from-slate-900 via-[#101423] to-slate-950 flex flex-col items-center justify-center p-4 text-center space-y-2 border-b border-white/5">
        ...
        <button
          ...
          className={`absolute bottom-2.5 right-2.5 p-2 rounded-lg backdrop-blur-md transition-all z-10 ${
            isBookmarked(item.id)
              ? 'bg-amber-500 text-black shadow-md'
              : 'bg-black/60 text-slate-300 hover:text-white hover:bg-black/80'
          }`}
          title={isBookmarked(item.id) ? 'Remove Bookmark' : 'Bookmark Item'}
        >
