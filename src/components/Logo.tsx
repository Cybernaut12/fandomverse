    ...
    <div className={`flex items-center space-x-3 select-none group ${className}`}>
      ...
      <div className={`relative ${currentSize.icon} rounded-xl overflow-hidden p-[1.5px] bg-gradient-to-tr from-[#e8a87c] via-purple-500 to-[#f5d1ba] shadow-lg shadow-[#e8a87c]/20 group-hover:shadow-[#e8a87c]/40 group-hover:scale-105 transition-all duration-300`}>
        {useImage ? (
          ...
          <div className="w-full h-full bg-[#090b12] rounded-[11px] p-1 flex items-center justify-center relative overflow-hidden">
            ...
            <img 
              ...
              className="w-full h-full object-contain relative z-10 drop-shadow-[0_0_8px_rgba(232,168,124,0.5)]"
            />
          </div>
        )}
      </div>
      ...
      {showWordmark && (
        <div className="flex items-center space-x-1.5">
          ...
          {showBadge && (
            <span className={`hidden sm:inline-block ml-1.5 uppercase font-mono font-extrabold tracking-wider rounded-md bg-[#e8a87c]/20 text-[#e8a87c] border border-[#e8a87c]/35 shadow-sm shadow-[#e8a87c]/10 ${currentSize.badge}`}>
              2.0
            </span>
          )}
        </div>
