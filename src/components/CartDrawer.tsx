...
import { X, Trash2, Plus, Minus, ShoppingBag, ArrowRight, Tag, ShieldAlert, Sparkles, Check } from 'lucide-react';
...
export const CartDrawer: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        <div 
          className="w-screen max-w-md glass-panel border-l border-white/10 shadow-2xl flex flex-col justify-between"
        ...
        >
            ...
            {cart.length === 0 ? (
              ...
              <>
                ...
                {cart.map(({ product, quantity }) => (
                  <div key={product.id} className="p-3.5 rounded-xl glass-card border border-white/10 flex items-center space-x-3">
                    <img
                      src={product.image}
                      alt={product.name}
                ...
                <form onSubmit={handleApplyPromo} className="pt-2">
                  <div className="flex space-x-2">
                    <input
                      type="text"
                      placeholder="Coupon (e.g. TECHWIZ7)"
                      value={promoCode}
                      onChange={(e) => setPromoCode(e.target.value)}
                      className="flex-1 px-3 py-2 rounded-xl bg-black/40 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400 font-mono uppercase"
                    />
                    <button
                      type="submit"
                      className="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-xs font-medium text-white transition-colors"
                    >
                      Apply
                    </button>
                  </div>
                  {promoMessage && (
                    <p className={`text-[11px] mt-1.5 font-mono ${discountPercent > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                      {promoMessage}
                    </p>
                  )}
                </form>
          ...
          {cart.length > 0 && (
            <div className="p-5 border-t border-white/10 bg-black/40 space-y-3">
              ...
              <button
                disabled
                className="w-full py-3 rounded-xl bg-slate-800 text-slate-400 text-xs font-bold uppercase tracking-wider cursor-not-allowed border border-white/10"
              >
