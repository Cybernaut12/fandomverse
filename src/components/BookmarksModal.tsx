...
export const BookmarksModal: React.FC = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'media' | 'characters' | 'events'>('all');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div 
        className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl glass-panel border border-white/10 shadow-2xl p-6 text-slate-200 flex flex-col justify-between"
      ...
      >
        ...
        <div className="py-4 flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.08]">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-slate-200 text-xs font-medium transition-all flex items-center space-x-1.5 disabled:opacity-50"
            ...
            <button
              ...
              disabled={totalCount === 0}
              className="px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold shadow-md shadow-purple-600/30 transition-all flex items-center space-x-1.5 disabled:opacity-50"
          ...
          {totalCount === 0 ? (
            ...
            <>
              ...
              {(activeFilter === 'all' || activeFilter === 'media') && bookmarkedMedia.map(item => {
                ...
                return (
                  <div key={item.id} className="p-4 rounded-xl glass-card border border-white/10 space-y-3">
                      ...
                      {editingNoteId === item.id ? (
                        <div className="flex items-center space-x-2">
                          <input
                            ...
                            placeholder="Add your session note (e.g. remember chapter 45)..."
                            className="flex-1 px-3 py-1.5 rounded-lg bg-black/50 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-amber-400"
                            autoFocus
                          />
                          <button
              ...
              {/* Characters Items */}
              {(activeFilter === 'all' || activeFilter === 'characters') && bookmarkedCharacters.map(char => (
                <div key={char.id} className="p-3.5 rounded-xl glass-card border border-white/10 flex items-center justify-between">
                  <div 
                    className="flex items-center space-x-3 cursor-pointer group"
                    onClick={() => {
                      setIsBookmarksModalOpen(false);
        ...
        <div className="pt-3 border-t border-white/10 flex items-center justify-between text-[11px] text-slate-400">
          ...
          <button
            onClick={() => setIsBookmarksModalOpen(false)}
            className="px-4 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-white text-xs font-medium"
          >
            Done
          </button>
        </div>
