...
export const ArticleDetailModal = () => {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl glass-panel border border-white/10 shadow-2xl p-6 sm:p-8 text-slate-200 space-y-6" onClick={(e) => e.stopPropagation()}>
        {/* Close Button */}
        <button onClick={() => setSelectedArticle(null)} className="absolute top-4 right-4 p-2 rounded-full bg-white/10 hover:bg-white/20 text-slate-300 hover:text-white transition-all">
          <X className="w-5 h-5"/>
        </button>

        {/* Cover Image */}
        <div className="relative aspect-[21/9] w-full rounded-xl overflow-hidden">
          <img src={article.coverImage} alt={article.title} className="w-full h-full object-cover filter brightness-[0.85]"/>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2 pb-3 border-b border-white/10 text-xs text-slate-400">
            <div className="flex items-center space-x-2.5">
              <img src={article.authorAvatar} alt={article.author} className="w-8 h-8 rounded-full object-cover ring-1 ring-purple-500/50"/>
              <div>
        ...
        <div className="flex flex-wrap gap-1.5 pt-2">
          {article.tags.map(tag => (<span key={tag} className="text-xs px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-slate-300 font-mono">
              #{tag}
            </span>))}
        </div>

        {/* Related Titles Spotlight */}
        {relatedTitles.length > 0 && (<div className="pt-4 border-t border-white/10 space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">
              Related Fandoms Mentioned in this Feature
            </h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {relatedTitles.map(item => (<div key={item.id} onClick={() => {
                  ...
                  <img src={item.coverImage} alt={item.title} className="w-12 h-16 rounded-md object-cover flex-shrink-0"/>
