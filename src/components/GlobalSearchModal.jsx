...
export const GlobalSearchModal = () => {
    const { isSearchModalOpen, setIsSearchModalOpen, mediaList, characters, articles, merchandise, events, setSelectedMedia, setSelectedCharacter, setSelectedArticle, setCurrentView } = useApp();
    ...
    const matchingCharacters = (selectedFilter === 'all' || selectedFilter === 'character')
        ? characters.filter(char => !cleanQuery ||
    ...
    const matchingArticles = (selectedFilter === 'all' || selectedFilter === 'article')
        ? articles.filter(art => !cleanQuery ||
    ...
    const matchingMerch = (selectedFilter === 'all' || selectedFilter === 'merch')
        ? merchandise.filter(m => !cleanQuery ||
    ...
    return (<div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-150">
      <div className="w-full max-w-3xl glass-panel border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]" onClick={(e) => e.stopPropagation()}>
        
        {/* Search Bar Input */}
        <div className="p-4 border-b border-white/10 flex items-center space-x-3 bg-black/40">
          <Search className="w-5 h-5 text-cyan-400 flex-shrink-0"/>
          <input type="text" placeholder="Search movies, anime, manga, games, characters, merchandise, articles..." value={query} onChange={(e) => setQuery(e.target.value)} className="flex-1 bg-transparent text-sm sm:text-base text-white placeholder-slate-400 focus:outline-none" autoFocus/>
          {query && (<button onClick={() => setQuery('')} className="p-1 rounded text-slate-400 hover:text-white">
              <X className="w-4 h-4"/>
            </button>)}
        ...
        ].map(f => (<button key={f.id} onClick={() => setSelectedFilter(f.id)} className={`px-2.5 py-1 rounded-lg whitespace-nowrap transition-colors ${selectedFilter === f.id
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {totalResults === 0 ? (<div className="text-center py-16 text-slate-500 space-y-2">
              ...
              {/* Media Results */}
              {matchingMedia.length > 0 && (<div className="space-y-2">
                  <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                    Titles & Media ({matchingMedia.length})
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {matchingMedia.slice(0, 6).map(item => (<div key={item.id} onClick={() => {
                        setIsSearchModalOpen(false);
              ...
              {/* Characters Results */}
              {matchingCharacters.length > 0 && (<div className="space-y-2">
                  <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                    Characters ({matchingCharacters.length})
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {matchingCharacters.slice(0, 4).map(char => (<div key={char.id} onClick={() => {
                        setIsSearchModalOpen(false);
              ...
              {/* Merchandise Results */}
              {matchingMerch.length > 0 && (<div className="space-y-2">
                  <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                    Merchandise ({matchingMerch.length})
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {matchingMerch.slice(0, 4).map(m => (<div key={m.id} onClick={() => {
                        setIsSearchModalOpen(false);
