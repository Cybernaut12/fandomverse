...
import { X, Quote, Bookmark } from 'lucide-react';
...
export const CharacterModal = () => {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl glass-panel border border-white/10 shadow-2xl p-6 text-slate-200 space-y-6" onClick={(e) => e.stopPropagation()}>
        {/* Close Button */}
        <button onClick={() => setSelectedCharacter(null)} className="absolute top-4 right-4 p-2 rounded-full bg-white/10 hover:bg-white/20 text-slate-300 hover:text-white transition-all">
          <X className="w-5 h-5"/>
        </button>

        {/* Character Header: Image + Basic Info */}
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 pt-2">
          <img src={char.image} alt={char.name} className="w-28 h-28 sm:w-36 sm:h-36 rounded-2xl object-cover ring-2 ring-purple-500/50 shadow-xl shadow-purple-500/10 flex-shrink-0"/>
          <div className="space-y-2 text-center sm:text-left flex-grow">
            ...
            <div>
              ...
              {char.japaneseName && (<p className="text-xs font-mono text-slate-400">{char.japaneseName}</p>)}
            </div>
            ...
            {char.actorOrVoice && (<p className="text-[11px] text-slate-400">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {char.traits.map((trait, idx) => (<span key={idx} className="text-xs px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-slate-200">
                {trait}
              </span>))}
          </div>
        </div>

        {char.powersOrSkills && char.powersOrSkills.length > 0 && (<div className="space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-cyan-400">Powers & Signature Skills</h4>
            <div className="flex flex-wrap gap-1.5">
              {char.powersOrSkills.map((power, idx) => (<span key={idx} className="text-xs px-2.5 py-1 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-cyan-300 font-mono">
                  {power}
                </span>))}
            </div>
          </div>)}
        ...
        {/* Memorable Quotes */}
        {char.quotes && char.quotes.length > 0 && (<div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/20 space-y-2">
            <div className="flex items-center space-x-1.5 text-purple-300 text-xs font-semibold">
              <Quote className="w-3.5 h-3.5"/>
              <span>Memorable Quotes</span>
            </div>
            {char.quotes.map((q, idx) => (<p key={idx} className="text-xs italic text-purple-200">
                "{q}"
              </p>))}
          </div>)}
        ...
        <div className="pt-2 flex justify-end">
          <button onClick={() => toggleBookmark(char.id)} className={`px-4 py-2 rounded-xl text-xs font-medium border transition-all flex items-center space-x-2 ${isBookmarked(char.id)
