...
import { ArrowLeft, Home, ChevronRight, CornerUpLeft } from 'lucide-react';
import { useApp, type AppView } from '../context/AppContext';

export const BackNavigationBar: React.FC = () => {
  const { 
    currentView, 
    setCurrentView, 
    activeCategory, 
    ...
    <div className="w-full mb-6 animate-in fade-in duration-200">
      <div className="flex flex-wrap items-center justify-between gap-3 px-3.5 py-2.5 rounded-2xl bg-white/[0.03] border border-white/[0.08] backdrop-blur-md shadow-lg">
        ...
        <div className="flex items-center space-x-2">
          <button
            onClick={goBack}
            className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 text-purple-200 hover:text-white border border-purple-500/25 hover:border-purple-500/40 transition-all group text-xs font-semibold shadow-sm active:scale-95"
            title="Go back to previous screen (Keyboard: Esc / Backspace)"
          >
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-white/[0.04] hover:bg-white/10 text-slate-400 hover:text-white border border-white/[0.06] transition-colors"
