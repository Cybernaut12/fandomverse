...
import { X, ArrowLeft, Star, Bookmark, Play, Share2, BookOpen, Users, Award, ExternalLink, ChevronRight, ChevronLeft } from 'lucide-react';
import { useApp } from '../context/AppContext';
export const MediaDetailModal = () => {
    const { selectedMedia, setSelectedMedia, toggleBookmark, isBookmarked, userProgress, updateProgressUnits, setProgressStatus, setUserRating, reviews, setIsReviewModalOpen, setSelectedVideoClip, setSelectedCharacter, characters, mediaList } = useApp();
    ...
    const franchiseCharacters = characters.filter(c => c.series.toLowerCase().includes(item.title.toLowerCase()) ||
        item.title.toLowerCase().includes(c.series.toLowerCase()) ||
        c.category === item.category).slice(0, 5);
    const handleShare = () => {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 overflow-y-auto bg-black/85 backdrop-blur-md animate-in fade-in duration-200" onClick={() => setSelectedMedia(null)} role="dialog" aria-modal="true">
      <div className="relative w-full max-w-5xl max-h-[92vh] overflow-y-auto rounded-2xl glass-panel border border-white/10 shadow-2xl text-slate-200" onClick={(e) => e.stopPropagation()}>
        {/* Sticky Top Bar Controls */}
        <div className="absolute top-4 inset-x-4 z-30 flex items-center justify-between pointer-events-none">
          <button onClick={() => setSelectedMedia(null)} className="pointer-events-auto inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-full bg-black/70 hover:bg-black/90 text-slate-200 hover:text-white border border-white/15 backdrop-blur-md transition-all shadow-lg text-xs font-semibold group" title="Go back (Esc)">
          ...
          <button onClick={() => setSelectedMedia(null)} className="pointer-events-auto p-2 rounded-full bg-black/70 hover:bg-red-500/30 text-slate-200 hover:text-red-300 border border-white/15 hover:border-red-500/40 backdrop-blur-md transition-all shadow-lg" aria-label="Close modal" title="Close (Esc)">
            <X className="w-5 h-5"/>
          </button>
        </div>
        ...
        <div className="relative w-full h-72 sm:h-96 overflow-hidden">
          <img src={item.bannerImage} alt={item.title} onError={(e) => {
            e.target.src = item.coverImage;
          ...
          <div className="absolute bottom-6 left-6 right-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div className="space-y-2 max-w-2xl">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {item.genres.map((g) => (<span key={g} className="px-2 py-0.5 rounded-md bg-white/10 text-slate-300 text-[11px] backdrop-blur-sm">
                    {g}
                  </span>))}
              </div>
              ...
              <div className="space-y-1">
                ...
                {item.originalTitle && (<p className="text-sm sm:text-base font-medium text-slate-400 font-mono">
                    {item.originalTitle}
                  </p>)}
              </div>
            ...
            <div className="flex items-center gap-2.5">
              {isPrintMedia ? (<button onClick={() => {
                ...
                </button>) : (<button onClick={handleTrailerClick} className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-semibold shadow-lg shadow-purple-600/30 transition-all flex items-center space-x-2 active:scale-95">
                  <Play className="w-4 h-4 fill-white"/>
                  <span>Watch Trailer</span>
                </button>)}

              <button onClick={() => toggleBookmark(item.id)} className={`p-2.5 rounded-xl border transition-all ${isBookmarked(item.id)
            ? 'bg-amber-500 text-black border-amber-500'
            : 'glass-panel text-slate-300 hover:text-white border-white/10 hover:bg-white/10'}`} title="Bookmark">
                <Bookmark className={`w-4 h-4 ${isBookmarked(item.id) ? 'fill-current' : ''}`}/>
              </button>

              <button onClick={handleShare} className="p-2.5 rounded-xl glass-panel text-slate-300 hover:text-white border-white/10 hover:bg-white/10 transition-all relative" title="Share Title">
                <Share2 className="w-4 h-4"/>
                {copiedLink && (<span className="absolute -top-7 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded bg-black text-[10px] text-cyan-400 border border-cyan-400/40 whitespace-nowrap">
                    Link Copied!
                  </span>)}
              </button>
            </div>
        ...
        ].map(tab => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`py-3.5 px-3 text-xs font-semibold transition-all border-b-2 whitespace-nowrap ${activeTab === tab.id
        ...
        <div className="p-6">
          ...
          {activeTab === 'overview' && (<div className="space-y-6">
              ...
              {item.quote && (<div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/20 text-purple-200 text-xs sm:text-sm font-medium italic">
                  "{item.quote}"
                </div>)}
              ...
              <div className="p-4 rounded-xl glass-card border border-white/10 space-y-3">
                ...
                : 'Not yet in your progress list'}
                  ...
                  {progress && (<div className="flex items-center space-x-2">
                      <button onClick={() => updateProgressUnits(item.id, 1)} className="px-3 py-1.5 rounded-lg bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-500/30 text-xs font-mono font-semibold">
                        +1 {progress.unitType.slice(0, -1)}
                      </button>
                      <button onClick={() => setProgressStatus(item.id, 'completed')} className="px-3 py-1.5 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/30 text-xs font-medium">
                        Mark Complete
                      </button>
                    </div>)}
                </div>

                {progress && (<div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-purple-500 to-cyan-400 transition-all duration-300" style={{ width: `${Math.min(100, Math.round((progress.currentUnits / progress.totalUnits) * 100))}%` }}/>
                  </div>)}
              </div>

              {/* Conditional Section: Comic/Manga Interactive Page Preview VS Video Trailer */}
              {isPrintMedia ? (<div id="comic-manga-preview" className="space-y-4 pt-2">
                  ...
                  <div className="relative w-full rounded-2xl overflow-hidden glass-card border border-white/10 bg-slate-950/80 p-3 sm:p-5 flex flex-col items-center">
                    ...
                    <div className="absolute inset-y-0 left-2 sm:left-4 z-10 flex items-center">
                      <button onClick={() => setPreviewPageIndex(prev => (prev === 0 ? 2 : prev - 1))} className="p-2.5 rounded-full bg-black/75 hover:bg-black/95 text-white/80 hover:text-white border border-white/20 backdrop-blur-md shadow-xl transition-all active:scale-95" title="Previous page" aria-label="Previous page">
                        <ChevronLeft className="w-5 h-5"/>
                      </button>
                    </div>
                    <div className="absolute inset-y-0 right-2 sm:right-4 z-10 flex items-center">
                      <button onClick={() => setPreviewPageIndex(prev => (prev === 2 ? 0 : prev + 1))} className="p-2.5 rounded-full bg-black/75 hover:bg-black/95 text-white/80 hover:text-white border border-white/20 backdrop-blur-md shadow-xl transition-all active:scale-95" title="Next page" aria-label="Next page">
                    ...
                    : previewPageIndex === 1
                      ...
                      <div className="relative w-full flex justify-center items-center rounded-xl overflow-hidden bg-black/50 border border-white/10 shadow-2xl min-h-[300px] sm:min-h-[420px] max-h-[500px]">
                        <img src={previewPageIndex === 1 ? item.bannerImage : item.coverImage} alt={`${item.title} Preview Page ${previewPageIndex + 1}`} className={`w-full h-full ${previewPageIndex === 1 ? 'object-cover sm:object-contain' : 'object-contain'} max-h-[480px] transition-all duration-300`}/>
                ...
                ].map(tab => (<button key={tab.idx} onClick={() => setPreviewPageIndex(tab.idx)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${previewPageIndex === tab.idx
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={handleTrailerClick} className="text-xs text-cyan-400 hover:underline flex items-center space-x-1">
                  ...
                  <div className="relative aspect-video w-full rounded-xl overflow-hidden glass-card border border-white/10">
                    <iframe src={item.trailerUrl || "https://www.youtube.com/embed/S8_YwFLCh4U"} title={item.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="w-full h-full border-0"/>
                  </div>
                </div>)}
          ...
          {activeTab === 'arcs' && item.topArcs && (<div className="space-y-4">
              <p className="text-xs text-slate-400">
                Chronological sagas and milestone story arcs rated by the FandomVerse community.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {item.topArcs.map((arc, idx) => (<div key={idx} className="p-4 rounded-xl glass-card border border-white/10 space-y-2 hover:border-cyan-400/40 transition-all">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-white">{arc.title}</span>
                      {arc.status && (<span className="text-[10px] px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 font-mono">
                          {arc.status}
                        </span>)}
                    </div>
          ...
          {activeTab === 'cast' && (<div className="space-y-4">
              <p className="text-xs text-slate-400">
                Major characters, voice artists, and heroes from {item.title}.
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                {franchiseCharacters.map(char => (<div key={char.id} onClick={() => setSelectedCharacter(char)} className="p-3 rounded-xl glass-card border border-white/10 text-center space-y-2 cursor-pointer hover:border-cyan-400/50 hover:scale-105 transition-all">
                    <img src={char.image} alt={char.name} className="w-16 h-16 mx-auto rounded-full object-cover ring-2 ring-purple-500/40"/>
                    <div>
          ...
          {activeTab === 'reviews' && (<div className="space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-white/10">
                ...
                <button onClick={() => setIsReviewModalOpen(true)} className="px-3 py-1.5 rounded-lg bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-500/30 text-xs font-medium">
                  Write a Review
                </button>
              </div>

              {itemReviews.length === 0 ? (<div className="text-center py-8 text-xs text-slate-400 space-y-2">
                  <p>No community reviews yet for {item.title}.</p>
                  <button onClick={() => setIsReviewModalOpen(true)} className="text-cyan-400 underline font-medium">
                    Be the first fan to review this title!
                  </button>
                </div>) : (<div className="space-y-3">
                  {itemReviews.map(rev => (<div key={rev.id} className="p-4 rounded-xl glass-card border border-white/10 space-y-2">
                      <div className="flex items-center justify-between">
          ...
          {activeTab === 'similar' && (<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {relatedMedia.map(rel => (<div key={rel.id} onClick={() => setSelectedMedia(rel)} className="p-2.5 rounded-xl glass-card border border-white/10 cursor-pointer hover:border-cyan-400/50 hover:scale-105 transition-all space-y-2">
                  <img src={rel.coverImage} alt={rel.title} className="w-full aspect-[3/4] rounded-lg object-cover"/>
                  <div>
