...
import type { FandomDebate, CategoryType, DebateArgument } from '../types';
...
export const CommunityView: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'trending' | 'votes' | 'arguments' | 'newest'>('trending');
  
  // Expanded debate ID (defaults to first featured debate)
  const [activeDebateId, setActiveDebateId] = useState<string>(() => {
    return debates.find(d => d.featured)?.id || debates[0]?.id || '';
  });

  // Stance filter for arguments in active debate
  const [argumentStanceFilter, setArgumentStanceFilter] = useState<'ALL' | 'A' | 'B'>('ALL');

  // New argument form state
  const [showArgForm, setShowArgForm] = useState(false);
  const [argStance, setArgStance] = useState<'A' | 'B'>('A');
  ...
  const [newDebateCategory, setNewDebateCategory] = useState<CategoryType>('anime');
  ...
  const filteredDebates = useMemo(() => {
    return debates.filter(d => {
      ...
      if (sortBy === 'trending') return (b.featured ? 1 : 0) - (a.featured ? 1 : 0) || totalVotesB - totalVotesA;
      if (sortBy === 'votes') return totalVotesB - totalVotesA;
      if (sortBy === 'arguments') return b.arguments.length - a.arguments.length;
  ...
  const handleAddArgument = (e: React.FormEvent) => {
    e.preventDefault();
    if (!activeDebate || !argTitle.trim() || !argContent.trim()) return;
  ...
  const handleCreateDebateSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newDebateTitle.trim() || !newDebateStanceATitle.trim() || !newDebateStanceBTitle.trim()) return;
  ...
  return (
    <div className="space-y-10 pb-24 animate-in fade-in duration-300">
      ...
      <section className="relative rounded-3xl overflow-hidden border border-white/10 bg-gradient-to-br from-[#111320] via-[#0d0f17] to-[#170e24] p-6 sm:p-10 shadow-2xl">
        ...
        <div className="relative z-10 max-w-4xl space-y-4">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3">
            <button
              onClick={() => setIsNewDebateModalOpen(true)}
              className="inline-flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-rose-600 via-purple-600 to-indigo-600 hover:from-rose-500 hover:to-indigo-500 text-white font-medium text-xs sm:text-sm shadow-lg shadow-purple-600/30 transition-transform active:scale-95"
            >
      ...
      {formSuccessMessage && (
        <div className="p-4 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-xs sm:text-sm flex items-center space-x-2.5 animate-in fade-in duration-200">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search matchups, franchises, lore points, characters..."
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-rose-500 transition-colors"
            />
            {searchQuery && (
          ...
          <div className="flex items-center space-x-2 shrink-0">
            ...
            <select
              ...
              className="bg-[#121420] border border-white/10 text-slate-200 text-xs rounded-xl px-3 py-2 focus:outline-none focus:border-rose-500"
            >
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="rounded-3xl border border-white/15 bg-[#0e101a] overflow-hidden shadow-2xl relative">
              ...
              <div className="relative h-56 sm:h-72 w-full overflow-hidden">
                <img 
                  src={activeDebate.banner} 
                  alt={activeDebate.title}
                ...
                <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
                  <div className="flex flex-wrap gap-1.5">
                    {activeDebate.platforms.map((p, idx) => (
                  ...
                  {activeDebate.featured && (
                    <span className="px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 text-[10px] font-bold uppercase tracking-wider flex items-center space-x-1">
              ...
              <div className="p-6 space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className={`p-4 rounded-2xl border transition-all ${
                          ...
                          <div className="flex items-start justify-between gap-2 mb-2">
                            ...
                            {myVote === 'A' && (
                              <span className="px-2 py-0.5 rounded-full bg-cyan-500/20 text-cyan-300 text-[10px] font-bold flex items-center space-x-1">
                          ...
                          <button
                            onClick={() => voteDebate(activeDebate.id, 'A')}
                        ...
                        <div className={`p-4 rounded-2xl border transition-all ${
                          ...
                          <div className="flex items-start justify-between gap-2 mb-2">
                            ...
                            {myVote === 'B' && (
                              <span className="px-2 py-0.5 rounded-full bg-rose-500/20 text-rose-300 text-[10px] font-bold flex items-center space-x-1">
                          ...
                          <button
                            onClick={() => voteDebate(activeDebate.id, 'B')}
                            className={`w-full py-2 px-3 rounded-xl text-xs font-semibold flex items-center justify-center space-x-1.5 transition-all ${
                              myVote === 'B'
                ...
                <div className="pt-2 flex flex-wrap items-center gap-1.5 border-t border-white/10">
                  ...
                  {activeDebate.tags.map((t, i) => (
                    <span key={i} className="px-2 py-0.5 rounded-full bg-white/[0.04] text-[10px] text-slate-300 border border-white/[0.08]">
                      #{t}
                    </span>
                  ))}
                </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <button
                  onClick={() => setShowArgForm(!showArgForm)}
                  className="inline-flex items-center space-x-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-rose-600 to-purple-600 hover:from-rose-500 hover:to-purple-500 text-white text-xs font-semibold shadow-md transition-all self-start sm:self-auto"
                >
                  <PlusCircle className="w-4 h-4" />
              ...
              {showArgForm && (
                <form 
                  ...
                  className="p-5 rounded-2xl bg-[#121422] border border-rose-500/30 shadow-xl space-y-4 animate-in fade-in zoom-in-95 duration-200"
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="sm:col-span-2 space-y-1">
                      ...
                      <input 
                        ...
                        placeholder="e.g. Speed of Light Feats directly contradict planetary scaling"
                    ...
                    <div className="space-y-1">
                      ...
                      <select
                        value={argBadge}
                        onChange={(e) => setArgBadge(e.target.value)}
                        className="w-full px-3 py-2 rounded-xl bg-[#0e101a] border border-white/10 text-xs text-slate-300 focus:outline-none focus:border-rose-500"
                      >
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      placeholder="Lay down your logical thesis, feats, timestamps, or quotes that support your stance..."
                      className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      placeholder="e.g. Manga Chapter 1044 / Action Comics Vol 2 #775 / Patch 1.12 Lore"
                      className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500"
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-end space-x-2 pt-2">
                    ...
                    <button
                      type="submit"
                      className="px-5 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-semibold flex items-center space-x-1.5 shadow-md shadow-rose-600/30"
                    >
              ...
              <div className="flex items-center space-x-2 border-b border-white/10 pb-2">
                <button
                  onClick={() => setArgumentStanceFilter('ALL')}
                  className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${
                    argumentStanceFilter === 'ALL'
                ...
                <button
                  onClick={() => setArgumentStanceFilter('A')}
                  className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${
                    argumentStanceFilter === 'A'
                ...
                <button
                  onClick={() => setArgumentStanceFilter('B')}
                  className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${
                    argumentStanceFilter === 'B'
              ...
              {/* Arguments List */}
              <div className="space-y-4">
                {displayedArguments.length === 0 ? (
                  <div className="text-center py-10 p-6 rounded-2xl bg-white/[0.02] border border-white/5">
                    <p className="text-slate-400 text-xs">No arguments logged for this stance yet. Be the first to defend this side!</p>
                  </div>
                ) : (
                  displayedArguments.map(arg => {
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <div className="flex items-center space-x-2">
                                ...
                                {arg.badge && (
                                  <span className="px-2 py-0.2 rounded-full bg-white/10 text-[9px] font-semibold text-slate-300">
                                    {arg.badge}
                                  </span>
                                )}
                              </div>
                        ...
                        {arg.canonCitation && (
                          <div className="p-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-start space-x-2 text-xs mb-3">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-white/[0.06] text-xs">
                          <button
                            onClick={() => likeDebateArgument(activeDebate.id, arg.id)}
                            className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 hover:text-rose-400 transition-colors"
                          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative w-full max-w-2xl rounded-3xl bg-[#0f111c] border border-white/15 shadow-2xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto space-y-6">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              ...
              <button 
                onClick={() => setIsNewDebateModalOpen(false)}
            ...
            <form onSubmit={handleCreateDebateSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input 
                  ...
                  placeholder="e.g. Sukuna vs Gojo: Who dictated the Jujutsu pinnacle?"
                  className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => setNewDebateCategory(e.target.value as CategoryType)}
                    className="w-full px-3 py-2 rounded-xl bg-[#141624] border border-white/10 text-xs text-white focus:outline-none focus:border-rose-500"
                  >
                ...
                <div className="space-y-1">
                  ...
                  <input 
                    type="text" 
                    value={newDebatePlatforms}
                    onChange={(e) => setNewDebatePlatforms(e.target.value)}
                    placeholder="e.g. Manga, Shonen Jump, MAPPA Anime"
                    className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Provide context on why this collision matters to fans across both mediums..."
                  className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                ...
                <div className="p-3.5 rounded-xl bg-cyan-950/20 border border-cyan-500/30 space-y-2">
                  ...
                  <input
                    type="text"
                    required
                    value={newDebateStanceATitle}
                    onChange={(e) => setNewDebateStanceATitle(e.target.value)}
                    placeholder="Side A Title (e.g. Ryomen Sukuna)"
                    className="w-full px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500"
                  />
                  <textarea
                    rows={2}
                    value={newDebateStanceASummary}
                    onChange={(e) => setNewDebateStanceASummary(e.target.value)}
                    placeholder="Core claim of Side A..."
                    className="w-full px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500"
                  />
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-rose-950/20 border border-rose-500/30 space-y-2">
                  ...
                  <input
                    type="text"
                    required
                    value={newDebateStanceBTitle}
                    onChange={(e) => setNewDebateStanceBTitle(e.target.value)}
                    placeholder="Side B Title (e.g. Satoru Gojo)"
                    className="w-full px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500"
                  />
                  <textarea
                    rows={2}
                    value={newDebateStanceBSummary}
                    onChange={(e) => setNewDebateStanceBSummary(e.target.value)}
                    placeholder="Core claim of Side B..."
                    className="w-full px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input 
                    type="text" 
                    value={newDebateBanner}
                    onChange={(e) => setNewDebateBanner(e.target.value)}
                ...
                <div className="space-y-1">
                  ...
                  <input 
                    type="text" 
                    value={newDebateTags}
                    onChange={(e) => setNewDebateTags(e.target.value)}
                    placeholder="e.g. Jujutsu Kaisen, Domain Expansion, Shonen"
                    className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-end space-x-2 pt-4 border-t border-white/10">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-rose-600 to-purple-600 hover:from-rose-500 hover:to-purple-500 text-white text-xs font-semibold shadow-lg shadow-purple-600/30"
                >
