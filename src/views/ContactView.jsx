...
import { Mail, MapPin, Send, Navigation, Check } from 'lucide-react';
export const ContactView = () => {
    ...
    return (<div className="space-y-12 pb-16 animate-in fade-in duration-200">
        ...
        <div className="p-6 sm:p-8 rounded-2xl glass-card border border-white/10 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            <div className="space-y-1">
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Elena Rostova" className="w-full px-3 py-2.5 rounded-xl bg-black/40 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400" required/>
            </div>

            <div className="space-y-1">
              <label className="text-slate-400 font-medium">Email Address</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@domain.com" className="w-full px-3 py-2.5 rounded-xl bg-black/40 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400" required/>
            </div>

            <div className="space-y-1">
              <label className="text-slate-400 font-medium">Inquiry Topic</label>
              <select value={subject} onChange={(e) => setSubject(e.target.value)} className="w-full px-3 py-2.5 rounded-xl bg-black/40 border border-white/10 text-white focus:outline-none focus:border-cyan-400">
            ...
            <div className="space-y-1">
              ...
              <textarea rows={4} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Share your thoughts with the FandomVerse core team..." className="w-full px-3 py-2.5 rounded-xl bg-black/40 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400 leading-relaxed" required/>
            </div>

            {submitted && (<div className="p-3 rounded-xl bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs flex items-center space-x-2">
                <Check className="w-4 h-4 flex-shrink-0"/>
                <span>Thank you! Your message was received by our team.</span>
              </div>)}

            <button type="submit" className="w-full py-3 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold text-xs shadow-lg shadow-purple-600/30 transition-all flex items-center justify-center space-x-2">
        ...
        <div className="space-y-6">
          <div className="p-6 rounded-2xl glass-card border border-white/10 space-y-4">
            ...
            <div className="relative aspect-[4/3] w-full rounded-xl overflow-hidden border border-white/10 bg-[#0a0d14]">
              ...
              <div className={`w-full h-full relative transition-all duration-300 ${mapType === 'satellite' ? 'bg-[#0f141f]' : 'bg-[#0c0f18]'}`}>
                ...
                <div className="absolute bottom-3 right-3 flex flex-col space-y-1.5 z-10">
                  <button onClick={() => setMapZoom(z => Math.min(18, z + 1))} className="w-7 h-7 rounded-lg bg-black/80 hover:bg-black text-white text-xs font-bold border border-white/20 flex items-center justify-center transition-colors" title="Zoom in">
                    +
                  </button>
                  <button onClick={() => setMapZoom(z => Math.max(10, z - 1))} className="w-7 h-7 rounded-lg bg-black/80 hover:bg-black text-white text-xs font-bold border border-white/20 flex items-center justify-center transition-colors" title="Zoom out">
                    -
                  </button>
                </div>
                ...
                <div className="absolute top-3 right-3 flex items-center space-x-1 p-0.5 rounded-lg bg-black/80 border border-white/10 text-[10px] font-mono z-10">
                  <button onClick={() => setMapType('dark')} className={`px-2 py-0.5 rounded ${mapType === 'dark' ? 'bg-cyan-500 text-black font-bold' : 'text-slate-400'}`}>
                    Dark
                  </button>
                  <button onClick={() => setMapType('satellite')} className={`px-2 py-0.5 rounded ${mapType === 'satellite' ? 'bg-cyan-500 text-black font-bold' : 'text-slate-400'}`}>
                    Satellite
                  </button>
                </div>
            ...
            <div className="pt-2 flex items-center justify-between text-xs text-slate-400">
              ...
              <a href="https://maps.google.com" target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-white text-[11px] font-medium transition-colors flex items-center space-x-1">
