...
import { ShoppingBag, Star, Filter, Plus, Check, ShieldAlert, Sparkles, Tag } from 'lucide-react';
import { useApp } from '../context/AppContext';
import type { MerchandiseItem, CategoryType } from '../types';

export const MerchandiseView: React.FC = () => {
  ...
  const filteredMerch = merchandise.filter(item => {
    ...
    if (selectedType !== 'all' && item.itemType.toLowerCase() !== selectedType.toLowerCase()) return false;
    return true;
  });

  const handleAddToCart = (product: MerchandiseItem) => {
    addToCart(product);
      ...
      <div className="p-4 rounded-xl glass-card border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2 text-xs">
          ...
          <select
            ...
            className="px-3 py-1.5 rounded-lg bg-black/40 border border-white/10 text-white focus:outline-none focus:border-cyan-400"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {filteredMerch.map(item => {
          ...
          return (
            <div
              key={item.id}
              className="p-4 rounded-2xl glass-card border border-white/10 hover:border-cyan-400/50 flex flex-col justify-between space-y-4 group transition-all"
            >
              <div className="space-y-3">
                <div className="relative aspect-square rounded-xl overflow-hidden bg-slate-900">
                  <img
                    ...
                    alt={item.name}
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = 'https://files.cdn.printful.com/o/upload/product-catalog-img/fb/fb56069c088406ddf01d7b15b61e49d8_l';
                    }}
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                  />
                  {item.badge && (
                    <span className="absolute top-2.5 left-2.5 text-[10px] font-bold px-2 py-0.5 rounded bg-black/80 text-cyan-300 border border-cyan-400/30">
                      {item.badge}
                    </span>
                  )}
              ...
              <div className="pt-3 border-t border-white/[0.08] flex items-center justify-between">
                ...
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-all flex items-center space-x-1.5 active:scale-95 ${
                    isAdded
