...
import type { CategoryType, UserProgressItem, MediaItem } from '../types';

export const CollectionTrackerView: React.FC = () => {
  const { 
    currentView, 
    setCurrentView, 
    userProfile, 
  ...
  const [activeMode, setActiveMode] = useState<'collection' | 'progress'>(
    currentView === 'progress' ? 'progress' : 'collection'
  );

  // Collection filters (Screen 5)
  const [collectionCategory, setCollectionCategory] = useState<string>('all');
  const [collectionSearch, setCollectionSearch] = useState('');
  const [collectionSort, setCollectionSort] = useState<'recent' | 'title' | 'rating' | 'progress'>('recent');

  // Progress status filter (Screen 6)
  const [progressStatusFilter, setProgressStatusFilter] = useState<'all' | 'watching' | 'reading' | 'playing' | 'completed' | 'on-hold' | 'dropped'>('all');

  // Map progress items to media objects
  const trackedItemsList = Object.entries(userProgress).map(([mediaId, progress]) => {
  ...
  const filteredCollection = trackedItemsList.filter(({ media, progress }) => {
    if (collectionCategory !== 'all' && media.category !== collectionCategory) return false;
  ...
  const filteredProgress = trackedItemsList.filter(({ progress }) => {
    if (progressStatusFilter !== 'all' && progress.status !== progressStatusFilter) return false;
    return true;
  });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl glass-panel border border-white/10">
        <div className="flex items-center space-x-4">
          <img
            ...
            className="w-14 h-14 rounded-2xl object-cover ring-2 ring-purple-500/50 shadow-xl"
          />
          <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2 border-t border-white/[0.06]">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-1.5 rounded-xl bg-black/40 border border-white/10 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-purple-400"
                />
              </div>
            ...
            {filteredCollection.map(({ media, progress }) => {
              ...
              return (
                <div
                  ...
                  className="group rounded-xl glass-card border border-white/10 hover:border-cyan-400/50 cursor-pointer overflow-hidden flex flex-col justify-between transition-all hover:scale-[1.02]"
                >
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center space-x-2 overflow-x-auto scrollbar-none text-xs pb-1">
            {[
              ...
              <button
                key={tab.id}
                onClick={() => setProgressStatusFilter(tab.id as any)}
                className={`px-3.5 py-1.5 rounded-full whitespace-nowrap transition-colors flex items-center space-x-1.5 ${
                  progressStatusFilter === tab.id
                    ? 'bg-cyan-500 text-black font-bold shadow-md'
                    : 'bg-white/5 text-slate-400 hover:text-white hover:bg-white/10'
                }`}
              >
          ...
          <div className="space-y-3">
            {filteredProgress.map(({ media, progress }) => {
              ...
              return (
                <div
                  key={media.id}
                  className="p-4 rounded-xl glass-card border border-white/10 hover:border-white/20 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4"
                >
                  {/* Left: Poster + Title + Category */}
                  <div 
                    className="flex items-center space-x-3.5 cursor-pointer group flex-1"
                    onClick={() => setSelectedMedia(media)}
                  >
                    <img
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      onClick={() => updateProgressUnits(media.id, progress.unitType === 'hours' ? 5 : 1)}
                      className="px-3 py-1.5 rounded-lg bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-500/30 text-xs font-mono font-semibold transition-all"
                    >
                      +{progress.unitType === 'hours' ? '5 Hours' : `1 ${progress.unitType.slice(0, -1)}`}
                    </button>

                    <button
                      onClick={() => setProgressStatus(media.id, 'completed')}
                      disabled={progress.status === 'completed'}
                      className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white transition-colors disabled:opacity-30"
