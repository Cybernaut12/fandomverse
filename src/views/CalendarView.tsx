...
import { Calendar as CalendarIcon, Clock, Bell, BellRing, Filter, ChevronLeft, ChevronRight, Sparkles, Check } from 'lucide-react';
...
export const CalendarView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'month' | 'list' | 'today'>('month');
  const [selectedCategory, setSelectedCategory] = useState<string>('all');
  const [reminders, setReminders] = useState<Record<string, boolean>>({});
  const [selectedRelease, setSelectedRelease] = useState<CalendarRelease | null>(releases[0] || null);
    ...
    <div className="space-y-8 pb-16 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-2xl glass-panel border border-white/10">
        ...
        <div className="flex items-center p-1 rounded-xl bg-black/40 border border-white/10 self-start sm:self-auto">
          {(['month', 'list', 'today'] as const).map(tab => (
            <button
              key={tab}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-7 gap-1.5 sm:gap-2">
            ...
            {Array.from({ length: emptyDaysBefore }).map((_, idx) => (
              <div key={`empty-${idx}`} className="min-h-[90px] sm:min-h-[110px] rounded-xl bg-white/[0.01] border border-white/[0.03] opacity-40" />
            ))}
            ...
            {Array.from({ length: totalDaysInMonth }).map((_, idx) => {
              ...
              return (
                <div
                  key={dayNum}
                  className={`min-h-[90px] sm:min-h-[110px] p-2 rounded-xl glass-card border transition-all flex flex-col justify-between ${
                    isToday 
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {isToday && (
                      <span className="text-[9px] uppercase font-bold text-black bg-cyan-400 px-1 rounded font-mono">
                        Today
                      </span>
                    )}
                  </div>
                  ...
                  <div className="space-y-1 my-auto">
                    {dayReleases.map(rel => (
                      <div
                        ...
                        className="p-1 rounded-md bg-purple-500/20 hover:bg-purple-500/40 border border-purple-500/30 text-[10px] text-purple-200 cursor-pointer transition-colors leading-tight line-clamp-2"
                        title={`${rel.title} (${rel.platform})`}
                      >
          ...
          {selectedRelease && (
            <div className="p-5 rounded-2xl glass-panel border border-cyan-400/40 shadow-2xl flex flex-col sm:flex-row items-center sm:items-start justify-between gap-5 mt-6 animate-in fade-in">
              <img
                src={selectedRelease.coverImage}
                alt={selectedRelease.title}
                className="w-24 h-32 rounded-xl object-cover ring-2 ring-cyan-400/50 flex-shrink-0"
              />
        ...
        <div className="space-y-3">
          {filteredReleases.map(rel => {
            ...
            return (
              <div
                key={rel.id}
                className="p-4 rounded-xl glass-card border border-white/10 hover:border-white/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all"
              >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors flex items-center space-x-1.5 self-start sm:self-auto ${
                    hasReminder
      ...
      {/* ========================================================================= */}
      {activeTab === 'today' && (
        <div className="p-8 rounded-2xl glass-panel border border-cyan-400/40 text-center space-y-4">
