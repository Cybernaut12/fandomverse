...
import { Sparkles, Film, Tv, Gamepad2, BookOpen, Layers, Calendar, Image as ImageIcon, Play, Radio, Filter, ArrowUpDown } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { MediaCard } from '../components/MediaCard';
import { Breadcrumbs } from '../components/Breadcrumbs';
export const CategoryHubView = () => {
    const { activeCategory, setActiveCategory, mediaList, characters, events, articles, merchandise, galleries, audioClips, setSelectedMedia, setSelectedCharacter, setSelectedArticle, setSelectedVideoClip, setLightboxImage, addToCart, setCurrentView } = useApp();
    ...
    const filteredTitles = categoryMedia.filter(item => {
        if (selectedSubTag !== 'all' && !item.tags.includes(selectedSubTag))
            return false;
        return true;
    });
    // Sort titles
    ...
    return (<div className="space-y-10 pb-16 animate-in fade-in duration-200">
      ...
      <div className="p-4 rounded-xl glass-card border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        ].map(tab => (<button key={tab.id} onClick={() => setContentTypeFilter(tab.id)} className={`px-3 py-1.5 rounded-lg whitespace-nowrap transition-colors ${contentTypeFilter === tab.id
        ...
        <div className="flex items-center space-x-3 text-xs">
          {allSubTags.length > 0 && (<div className="flex items-center space-x-1.5">
              <Filter className="w-3.5 h-3.5 text-slate-400"/>
              <select value={selectedSubTag} onChange={(e) => setSelectedSubTag(e.target.value)} className="px-2.5 py-1.5 rounded-lg bg-black/40 border border-white/10 text-white focus:outline-none focus:border-cyan-400">
                <option value="all">All Sub-Tags</option>
                {allSubTags.map(tag => (<option key={tag} value={tag}>{tag}</option>))}
              </select>
            </div>)}

          <div className="flex items-center space-x-1.5">
            <ArrowUpDown className="w-3.5 h-3.5 text-slate-400"/>
            <select value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} className="px-2.5 py-1.5 rounded-lg bg-black/40 border border-white/10 text-white focus:outline-none focus:border-cyan-400">
      ...
      {(contentTypeFilter === 'all' || contentTypeFilter === 'titles') && (<section className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-white/[0.08]">
            ...
            <div className="flex items-center space-x-1.5 self-start sm:self-auto bg-black/40 p-1 rounded-xl border border-white/10">
              ...
              <button onClick={() => setCardSize('large')} title="Large Poster View" className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-all ${cardSize === 'large'
                ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md'
                : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                Large
              </button>
              <button onClick={() => setCardSize('standard')} title="Standard View" className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-all ${cardSize === 'standard'
                ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md'
                : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                Standard
              </button>
              <button onClick={() => setCardSize('compact')} title="Compact Density View" className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-all ${cardSize === 'compact'
                ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md'
                : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                Compact
              </button>
          ...
          <div className={`grid gap-3 sm:gap-4 transition-all duration-300 ${cardSize === 'large'
                ? 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4'
                : cardSize === 'compact'
                    ? 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7'
                    : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5'}`}>
            {filteredTitles.map(item => (<MediaCard key={item.id} item={item}/>))}
          </div>
        </section>)}

      {/* SECTION 2: CHARACTER PROFILES (At least 5 per category as per SRS page 11) */}
      {(contentTypeFilter === 'all' || contentTypeFilter === 'characters') && (<section className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-4">
            {categoryCharacters.map(char => (<div key={char.id} onClick={() => setSelectedCharacter(char)} className="group p-3.5 rounded-2xl glass-card border border-white/10 hover:border-purple-400/50 cursor-pointer text-center space-y-3 transition-all hover:scale-105">
      ...
      {(contentTypeFilter === 'all' || contentTypeFilter === 'galleries') && categoryGalleries.length > 0 && (<section className="space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-white/[0.08]">
            <div>
              <h2 className="text-base font-bold text-white tracking-tight flex items-center space-x-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {categoryGalleries.map(img => (<div key={img.id} onClick={() => setLightboxImage(img)} className="group relative aspect-video rounded-xl overflow-hidden glass-card border border-white/10 cursor-pointer">
      ...
      {(contentTypeFilter === 'all' || contentTypeFilter === 'media') && categoryAudioClips.length > 0 && (<section className="space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-white/[0.08]">
            <h2 className="text-base font-bold text-white tracking-tight flex items-center space-x-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {categoryAudioClips.map(clip => (<div key={clip.id} onClick={() => setSelectedVideoClip(clip)} className="p-3.5 rounded-xl glass-card border border-white/10 hover:border-purple-400/50 cursor-pointer flex items-center space-x-3.5 group transition-all">
      ...
      {(contentTypeFilter === 'all' || contentTypeFilter === 'events') && (<section className="space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-white/[0.08]">
            <div>
              <h2 className="text-base font-bold text-white tracking-tight flex items-center space-x-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {categoryEvents.map(evt => (<div key={evt.id} className="p-4 rounded-xl glass-card border border-white/10 space-y-3">
                ...
                {evt.attendeesCount && (<div className="pt-2 border-t border-white/[0.06] text-[10px] text-slate-400 font-mono">
