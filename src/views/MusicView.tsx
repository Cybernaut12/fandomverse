...
export const MusicView: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'popularity' | 'tempo' | 'energy' | 'duration'>('popularity');
  const [onlyFavorites, setOnlyFavorites] = useState(false);
  const [inspectTrack, setInspectTrack] = useState<SpotifyTrack | null>(null);
  ...
  return (
    <div className="space-y-8 pb-32 animate-fade-in">
      ...
      {spotlightTrack && (
        <div className="glass-panel border border-white/10 rounded-3xl overflow-hidden p-6 lg:p-8 bg-gradient-to-br from-slate-900/90 via-[#0d0f17]/90 to-purple-950/20 relative">
          <div className="flex flex-col md:flex-row items-center gap-6 lg:gap-8">
            ...
            <div className="relative group w-44 h-44 sm:w-52 sm:h-52 rounded-2xl overflow-hidden shadow-2xl shadow-purple-900/30 flex-shrink-0">
              <img 
                src={spotlightTrack.album_cover} 
                alt={spotlightTrack.album_name}
              ...
              {currentPlayingTrack?.track_id === spotlightTrack.track_id && isPlaying && (
                <div className="absolute bottom-2 left-2 right-2 flex items-end justify-center space-x-1 h-6 bg-black/60 backdrop-blur-sm rounded-lg py-1 px-2">
            ...
            <div className="flex-1 space-y-4 w-full">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => toggleFavoriteTrack(spotlightTrack.track_id)}
                    className="p-2 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 hover:text-red-400 transition-colors"
                    title="Add to Favorites"
                  >
                    <Heart className={`w-4 h-4 ${isFavoriteTrack(spotlightTrack.track_id) ? 'fill-red-500 text-red-500' : ''}`} />
                  </button>
                  {spotlightTrack.spotify_url && (
                    <a
                      href={spotlightTrack.spotify_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-3 py-1.5 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/30 text-xs font-medium flex items-center space-x-1.5 transition-colors"
                    >
              ...
              {spotlightTrack.lyrics_snippet && (
                <div className="p-3 rounded-xl bg-white/[0.03] border border-white/5 text-xs text-slate-300 italic">
                  "{spotlightTrack.lyrics_snippet}"
                </div>
              )}
              ...
              <div className="flex items-center space-x-3 pt-2">
                ...
                <button
                  onClick={() => setInspectTrack(spotlightTrack)}
                  className="px-4 py-2.5 rounded-xl bg-white/[0.05] hover:bg-white/10 border border-white/10 text-slate-300 text-sm font-medium transition-colors flex items-center space-x-2"
                >
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-stretch md:items-center">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search by track, artist, album, franchise..."
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 focus:border-cyan-500 focus:outline-none text-sm text-white placeholder-slate-500 transition-colors"
            />
          </div>
        ...
        <div className="divide-y divide-white/5">
          {filteredTracks.map((track, idx) => {
            ...
            return (
              <div 
                key={track.track_id}
                className={`flex items-center justify-between px-4 sm:px-6 py-3.5 hover:bg-white/[0.03] transition-colors group ${
                  currentPlayingTrack?.track_id === track.track_id ? 'bg-cyan-500/[0.06] border-l-2 border-cyan-400' : ''
                }`}
              >
                {/* Index / Play Button */}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center space-x-2">
                      <span className={`text-sm font-semibold truncate ${
                        currentPlayingTrack?.track_id === track.track_id ? 'text-cyan-400' : 'text-white'
                      }`}>
                        {track.track_name}
                      </span>
                      {track.explicit && (
                        <span className="px-1 py-0.2 rounded bg-slate-700 text-slate-300 text-[9px] font-mono">E</span>
                      )}
                    </div>
          ...
          <div className="glass-panel border border-white/10 rounded-3xl p-6 sm:p-8 max-w-lg w-full bg-[#0d0f17] space-y-6">
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center space-x-3.5">
                <img 
                  src={inspectTrack.album_cover} 
                  alt={inspectTrack.album_name}
