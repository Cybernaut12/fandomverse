...
import { Sparkles, Flame, ArrowRight, Compass, Play, Film, Tv, Gamepad2, BookOpen, Layers, ShoppingBag, Clock, Swords, Headphones, Pause, ThumbsUp, Heart, Maximize2, Clapperboard, ListVideo, X } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { HeroCarousel } from '../components/HeroCarousel';
import { FeaturedFandomsCoverflow } from '../components/FeaturedFandomsCoverflow';
import { TrendingContinuousScroller } from '../components/TrendingContinuousScroller';
export const HomeView = () => {
    const { mediaList, characters, articles, events, merchandise, audioClips, setSelectedMedia, setSelectedCharacter, setSelectedArticle, setSelectedVideoClip, addToCart, navigateToCategory, setCurrentView, visitorCount, debates, userVotes, voteDebate, musicTracks, currentPlayingTrack, isPlaying, playTrack, togglePlayPause, isFavoriteTrack, toggleFavoriteTrack } = useApp();
    ...
    const [activeTrailerId, setActiveTrailerId] = useState('gta-vi');
    const [trailerFilter, setTrailerFilter] = useState('all');
    const [isInlineCinemaPlaying, setIsInlineCinemaPlaying] = useState(false);
    ...
    return (<div className="space-y-16 pb-12 animate-in fade-in duration-300">
      ...
      {/* 2. Trending This Week (Continuous Scroller) */}
      <TrendingContinuousScroller />

      {/* 3. Featured Across the Fandoms (3D Overlapping Coverflow Stack) */}
      <FeaturedFandomsCoverflow />

      {/* 4. Explore Category Hubs (7 Fandom Universes) */}
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          {categoriesConfig.map(cat => (<div key={cat.cat} onClick={() => navigateToCategory(cat.cat)} className="group p-5 rounded-2xl glass-card border border-white/10 hover:border-[#e8a87c]/50 cursor-pointer transition-all hover:scale-[1.02] flex flex-col justify-between space-y-4">
              <div className="flex items-center justify-between">
                <div className={`p-3 rounded-xl bg-gradient-to-tr ${cat.color} text-white shadow-lg shadow-black/40`}>
                  {cat.icon}
                </div>
                <span className="text-xs font-mono font-bold text-[#e8a87c] bg-[#e8a87c]/10 px-2 py-0.5 rounded-full border border-[#e8a87c]/20">
                  {cat.count} Titles
                </span>
              </div>

              <div>
                <h3 className="text-base font-bold text-white group-hover:text-[#e8a87c] transition-colors">
                  {cat.label}
                </h3>
              ...
              <div className="pt-2 border-t border-white/[0.06] flex items-center justify-between text-xs text-slate-400 group-hover:text-[#e8a87c] transition-colors">
                <span>Enter Hub</span>
      ...
      {spotlightDebate && (<section className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setCurrentView('community')} className="text-xs font-semibold text-rose-400 hover:text-rose-300 flex items-center space-x-1 group">
          ...
          <div className="rounded-3xl border border-rose-500/25 bg-[#0f111c] overflow-hidden shadow-2xl relative">
            <div className="relative h-44 sm:h-56 w-full overflow-hidden">
              <img src={spotlightDebate.banner} alt={spotlightDebate.title} className="w-full h-full object-cover brightness-75 scale-105 hover:scale-100 transition-transform duration-700"/>
              <div className="absolute inset-0 bg-gradient-to-t from-[#0f111c] via-[#0f111c]/70 to-transparent"/>
              
              <div className="absolute top-4 left-4 flex flex-wrap gap-1.5">
                {spotlightDebate.platforms.map((p, idx) => (<span key={idx} className="px-2.5 py-1 rounded-md bg-black/60 backdrop-blur-md text-[11px] font-semibold text-rose-300 border border-rose-500/30">
                    {p}
                  </span>))}
              </div>
            ...
            <div className="p-5 sm:p-6 space-y-4">
              {(() => {
                ...
                return (<div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                      <div className={`p-3.5 rounded-xl border transition-all ${myVote === 'A' ? 'bg-cyan-950/30 border-cyan-500 shadow-md ring-1 ring-cyan-500/50' : 'bg-white/[0.03] border-white/10'}`}>
                        ...
                        <button onClick={() => voteDebate(spotlightDebate.id, 'A')} className={`w-full py-1.5 px-3 rounded-lg text-xs font-semibold flex items-center justify-center space-x-1.5 transition-all ${myVote === 'A' ? 'bg-cyan-500 text-black' : 'bg-cyan-500/15 hover:bg-cyan-500/25 text-cyan-300 border border-cyan-500/30'}`}>
                          <ThumbsUp className="w-3 h-3"/>
                          <span>Vote Side A ({spotlightDebate.stanceA.votes})</span>
                        </button>
                      </div>

                      <div className={`p-3.5 rounded-xl border transition-all ${myVote === 'B' ? 'bg-rose-950/30 border-rose-500 shadow-md ring-1 ring-rose-500/50' : 'bg-white/[0.03] border-white/10'}`}>
                        <div className="flex items-center justify-between mb-1">
                        ...
                        <button onClick={() => voteDebate(spotlightDebate.id, 'B')} className={`w-full py-1.5 px-3 rounded-lg text-xs font-semibold flex items-center justify-center space-x-1.5 transition-all ${myVote === 'B' ? 'bg-rose-500 text-white' : 'bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border border-rose-500/30'}`}>
                          <ThumbsUp className="w-3 h-3"/>
              ...
              {secondaryDebates.length > 0 && (<div className="pt-3 border-t border-white/10 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {secondaryDebates.map(d => (<div key={d.id} onClick={() => setCurrentView('community')} className="p-2.5 rounded-xl bg-white/[0.02] hover:bg-white/[0.06] border border-white/5 hover:border-white/15 cursor-pointer flex items-center space-x-3 transition-colors group">
      ...
      {homeMusicTracks.length > 0 && (<section className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
            ...
            <button onClick={() => setCurrentView('music')} className="text-xs font-semibold text-emerald-400 hover:text-emerald-300 flex items-center space-x-1 group">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {homeMusicTracks.map(track => {
                ...
                return (<div key={track.track_id} className={`p-3.5 rounded-2xl border transition-all flex items-center space-x-3.5 group ${isThisPlaying
                  ...
                  <div className="relative shrink-0 w-16 h-16 rounded-xl overflow-hidden bg-slate-900 shadow-md">
                    <img src={track.album_cover} alt={track.track_name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>
                    <button onClick={() => {
                        if (isCurrent) {
                            togglePlayPause();
                        }
                    ...
                    }} className="absolute inset-0 bg-black/40 hover:bg-black/50 flex items-center justify-center transition-all" title={isThisPlaying ? "Pause Preview" : "Play Preview"}>
                      <div className="w-8 h-8 rounded-full bg-emerald-500 text-black flex items-center justify-center shadow-lg transition-transform hover:scale-110">
                        {isThisPlaying ? (<Pause className="w-4 h-4 fill-black"/>) : (<Play className="w-4 h-4 fill-black ml-0.5"/>)}
                      </div>
                    </button>
                  </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <h4 onClick={() => setCurrentView('music')} className="text-xs font-bold text-white hover:text-emerald-300 transition-colors truncate cursor-pointer">
                      {track.track_name}
                    </h4>
                  ...
                  {/* Like button */}
                  <button onClick={() => toggleFavoriteTrack(track.track_id)} className={`p-2 rounded-xl transition-colors shrink-0 ${isFav ? 'text-pink-500 hover:text-pink-400' : 'text-slate-500 hover:text-white hover:bg-white/5'}`} title={isFav ? "Saved to Favorites" : "Add to Favorites"}>
      ...
      <section className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 max-w-full custom-scrollbar">
            ...
            return (<button key={tab.id} onClick={() => setTrailerFilter(tab.id)} className={`px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all flex items-center space-x-1.5 ${isActive
                  ...
                  <span>{tab.label}</span>
                  <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-mono ${isActive ? 'bg-black/30 text-cyan-200' : 'bg-white/10 text-slate-400'}`}>
                    {count}
                  </span>
                </button>);
        })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-8 rounded-2xl overflow-hidden glass-panel border border-white/15 shadow-2xl bg-[#090b10] group">
            ...
            <div className="relative aspect-video w-full overflow-hidden bg-black">
              {isInlineCinemaPlaying ? (<div className="relative w-full h-full">
                  <iframe src={`${activeTrailer.embedUrl}${activeTrailer.embedUrl.includes('?') ? '&' : '?'}autoplay=1&enablejsapi=1`} title={activeTrailer.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="w-full h-full border-0"/>
                  {/* Floating Exit Overlay */}
                  <div className="absolute top-3 right-3 flex items-center space-x-2 z-10">
                    <button onClick={() => setIsInlineCinemaPlaying(false)} className="px-2.5 py-1.5 rounded-lg bg-black/80 hover:bg-red-500/80 text-white border border-white/20 text-xs font-semibold transition-all backdrop-blur-md flex items-center space-x-1.5 shadow-lg" title="Stop playing inline">
                      <X className="w-3.5 h-3.5"/>
                      <span>Close Video</span>
                    </button>
                  </div>
                </div>) : (<div className="relative w-full h-full cursor-pointer" onClick={() => setIsInlineCinemaPlaying(true)}>
                  {/* High-res 16:9 Poster with zoom on hover */}
                  <img src={activeTrailer.thumbnail} alt={activeTrailer.title} className="w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-700 ease-out"/>
                  {/* Vignette gradients */}
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center space-y-3">
                    <div className="relative group/play flex items-center justify-center">
                      <div className="absolute -inset-3 rounded-full bg-[#e8a87c]/25 blur-md animate-pulse"/>
                      <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-[#e8a87c] hover:bg-[#f0b992] text-[#0a0a0f] flex items-center justify-center shadow-2xl shadow-[#e8a87c]/50 group-hover:scale-110 active:scale-95 transition-all">
                        <Play className="w-7 h-7 sm:w-8 sm:h-8 fill-[#0a0a0f] ml-1 text-[#0a0a0f]"/>
                      </div>
                    </div>
            ...
            <div className="p-4 sm:p-5 space-y-3 bg-[#0a0c13]/90 border-t border-white/10">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div className="space-y-1.5 flex-1 min-w-0">
                  <div className="flex items-center space-x-2">
                    <span className="text-xs font-bold text-[#e8a87c] font-mono tracking-wider uppercase">
                  ...
                  {activeTrailer.tagline && (<p className="text-xs text-[#e8a87c]/90 italic font-serif">
                ...
                <div className="flex items-center flex-wrap sm:flex-nowrap gap-2 pt-1 sm:pt-0 flex-shrink-0">
                  {isInlineCinemaPlaying ? (<button onClick={() => setIsInlineCinemaPlaying(false)} className="px-3.5 py-2 rounded-xl bg-red-500/20 hover:bg-red-500/30 text-red-200 hover:text-white border border-red-500/40 text-xs font-semibold flex items-center space-x-1.5 transition-all active:scale-95">
                      <X className="w-3.5 h-3.5"/>
                      <span>Stop Video</span>
                    </button>) : (<button onClick={() => setIsInlineCinemaPlaying(true)} className="px-4 py-2 rounded-xl bg-[#e8a87c] hover:bg-[#f0b992] text-[#0a0a0f] text-xs font-bold shadow-md shadow-[#e8a87c]/25 flex items-center space-x-1.5 transition-all active:scale-95">
                      <Play className="w-3.5 h-3.5 fill-[#0a0a0f]"/>
        ...
        }} className="px-3.5 py-2 rounded-xl glass-panel hover:bg-white/10 text-slate-200 hover:text-white border border-white/15 text-xs font-medium flex items-center space-x-1.5 transition-all active:scale-95" title="Open Fullscreen Cinema Player (Press Esc to Leave)">
        ...
        }} className="px-3 py-2 rounded-xl glass-panel hover:bg-purple-500/20 text-purple-300 hover:text-purple-200 border border-purple-500/30 text-xs font-medium flex items-center space-x-1 transition-all" title="View Franchise Hub & Community">
          ...
          <div className="lg:col-span-4 rounded-2xl glass-panel border border-white/15 p-4 space-y-3.5 bg-[#0a0c13]/80">
            ...
            return (<div key={trailer.id} onClick={() => {
                    setActiveTrailerId(trailer.id);
                }} className={`p-2 rounded-xl border cursor-pointer transition-all flex items-center space-x-3 group ${isActive
                    ...
                    <div className="relative w-24 sm:w-28 aspect-video rounded-lg overflow-hidden bg-slate-900 flex-shrink-0 border border-white/10">
                      <img src={trailer.thumbnail} alt={trailer.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>
                      <div className={`absolute inset-0 flex items-center justify-center transition-all ${isActive ? 'bg-black/25' : 'bg-black/40 group-hover:bg-black/20'}`}>
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-transform ${isActive
                    ...
                    <div className="flex-1 min-w-0 space-y-0.5">
                      <div className="flex items-center space-x-1.5">
                        <span className={`text-[9px] uppercase font-bold px-1.5 py-0.2 rounded font-mono ${isActive
                    ? 'bg-cyan-400/25 text-cyan-200 border border-cyan-400/30'
                    : 'bg-white/10 text-slate-400'}`}>
                          {trailer.category.replace('-', ' ')}
                        </span>
                        {isActive && (<span className="inline-flex items-center space-x-1 text-[9px] font-bold text-cyan-400 uppercase tracking-wider">
                            <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-ping inline-block"/>
                            <span>Playing</span>
                          </span>)}
                      </div>
                      <h4 className={`text-xs font-bold line-clamp-1 leading-snug transition-colors ${isActive ? 'text-white font-extrabold' : 'text-slate-200 group-hover:text-white'}`}>
                        {trailer.franchise}
                      </h4>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {topArticles.map(article => (<div key={article.id} onClick={() => setSelectedArticle(article)} className="group p-4 rounded-2xl glass-card border border-white/10 hover:border-cyan-400/50 cursor-pointer flex flex-col justify-between space-y-4 transition-all">
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 sm:gap-4">
          {topCharacters.map(char => (<div key={char.id} onClick={() => setSelectedCharacter(char)} className="group p-3 rounded-2xl glass-card border border-white/10 hover:border-purple-400/50 cursor-pointer text-center space-y-2.5 transition-all hover:scale-105">
              <img src={char.image} alt={char.name} className="w-20 h-20 mx-auto rounded-full object-cover ring-2 ring-purple-500/40 group-hover:ring-cyan-400 transition-all shadow-lg"/>
              <div>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <div className="p-2 rounded-xl bg-[#e8a87c]/15 text-[#e8a87c] border border-[#e8a87c]/30">
              <ShoppingBag className="w-5 h-5 text-[#e8a87c]"/>
            </div>
            <div>
              <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight font-heading">
          ...
          <button onClick={() => setCurrentView('merchandise')} className="text-xs font-semibold text-[#e8a87c] hover:text-[#f0b992] flex items-center space-x-1 group">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {featuredMerch.map(item => (<div key={item.id} className="p-3.5 rounded-2xl glass-card border border-white/10 hover:border-[#e8a87c]/50 flex flex-col justify-between space-y-3 transition-all">
              <div className="space-y-2">
                <div className="relative aspect-square rounded-xl overflow-hidden bg-slate-900">
                  <img src={item.image} alt={item.name} className="w-full h-full object-cover"/>
                  {item.badge && (<span className="absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded bg-black/70 text-[#e8a87c] border border-[#e8a87c]/30">
                      {item.badge}
                    </span>)}
                </div>
                <div>
                  <span className="text-[10px] font-mono text-[#e8a87c] uppercase font-semibold">
              ...
              <div className="pt-2 border-t border-white/[0.06] flex items-center justify-between">
                <span className="text-sm font-bold text-[#e8a87c] font-mono">${item.price.toFixed(2)}</span>
                <button onClick={() => addToCart(item)} className="px-3 py-1.5 rounded-xl bg-[#e8a87c] hover:bg-[#f0b992] text-[#0a0a0f] text-xs font-bold shadow-md shadow-[#e8a87c]/25 transition-all active:scale-95">
                  + Add to Cart
                </button>
              </div>
            </div>))}
