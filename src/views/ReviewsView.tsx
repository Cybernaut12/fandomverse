...
import { MessageSquareQuote, Star, ThumbsUp, Plus, Filter, MessageCircle, Share2, Sparkles, ExternalLink } from 'lucide-react';
import { useApp } from '../context/AppContext';
import type { CategoryType, ReviewItem } from '../types';
    ...
    <div className="space-y-8 pb-16 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-2xl glass-panel border border-white/10">
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold text-xs shadow-lg shadow-purple-600/30 transition-all flex items-center space-x-2 self-start sm:self-auto active:scale-95"
        >
      ...
      <div className="flex items-center space-x-2 overflow-x-auto scrollbar-none text-xs pb-1">
        {[
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-full whitespace-nowrap transition-colors font-medium ${
      ...
      <div className="space-y-4">
        {filteredReviews.map(rev => {
          ...
          return (
            <div
              key={rev.id}
              className="p-5 sm:p-6 rounded-2xl glass-card border border-white/10 hover:border-white/20 transition-all flex flex-col md:flex-row items-start justify-between gap-5"
            >
              {/* Review Content */}
              <div className="space-y-3 flex-1">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <img
                      src={rev.avatar}
                      alt={rev.author}
                      className="w-9 h-9 rounded-full object-cover ring-1 ring-purple-500/50"
                    />
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="text-xs sm:text-sm font-bold text-white">{rev.author}</span>
                        {rev.badge && (
                          <span className="text-[10px] font-mono px-2 py-0.2 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/20">
                            {rev.badge}
                          </span>
                        )}
                      </div>
                ...
                <div className="pt-2 flex items-center space-x-4 text-xs text-slate-400">
                  <button
                    ...
                    className={`flex items-center space-x-1.5 hover:text-cyan-400 transition-colors ${isLiked ? 'text-cyan-400 font-bold' : ''}`}
                  >
              ...
              {media && (
                <div
                  ...
                  className="w-full md:w-36 rounded-xl glass-panel border border-white/10 p-2.5 flex md:flex-col items-center md:items-stretch space-x-3 md:space-x-0 md:space-y-2 cursor-pointer hover:border-cyan-400/50 hover:scale-105 transition-all flex-shrink-0"
                >
                  <img
                    src={media.coverImage}
                    alt={media.title}
                    className="w-12 h-16 md:w-full md:aspect-[3/4] rounded-lg object-cover"
                  />
                  <div>
