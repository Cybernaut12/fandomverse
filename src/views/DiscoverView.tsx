...
import { Search, Compass, Sparkles, Filter, SlidersHorizontal, Star, Flame, Eye } from 'lucide-react';
...
export const DiscoverView: React.FC = () => {
  ...
  const discoverFeatured = mediaList.filter(m => 
    m.id === 'arcane' || m.id === 'spider-man-spider-verse' || m.id === 'frieren-beyond-journeys-end'
  );
  ...
  const filteredItems = mediaList.filter(item => {
    ...
    // Year filter
    if (selectedYear !== 'all') {
      if (selectedYear === '2024' && item.year !== 2024) return false;
      if (selectedYear === '2023' && item.year !== 2023) return false;
      if (selectedYear === 'classic' && item.year > 2000) return false;
    }
    ...
    <div className="space-y-12 pb-12 animate-in fade-in duration-200">
      ...
      <div className="p-4 sm:p-5 rounded-2xl glass-panel border border-white/10 shadow-xl space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search for movies, shows, anime, manga, or games..."
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-black/50 border border-white/10 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-cyan-400 transition-colors"
          />
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5 text-xs">
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:border-cyan-400"
            >
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {discoverFeatured.map(item => (
            <div
              key={item.id}
              onClick={() => setSelectedMedia(item)}
              className="group relative rounded-2xl glass-panel border border-white/10 p-4 overflow-hidden cursor-pointer hover:border-cyan-400/50 hover:scale-[1.02] transition-all space-y-3"
            >
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {hiddenGems.map(item => (
            <MediaCard key={item.id} item={item} showProgress={false} />
          ))}
        </div>
      </section>
